...
public class BONUS_Program_UsingHashMap {
    ...
    private static BONUS_ConcordanceProcessor_UsingHashMap concordanceProcessor;
    ...
    private static void init(Scanner scanner) {
        ...
        while (!fileProcessed) try {
            ...
            if (workingFilePath != null) {
                concordanceProcessor = new BONUS_ConcordanceProcessor_UsingHashMap(new File(workingFilePath), chooseOutputPath(scanner));
                fileProcessed = true;
                concordanceProcessor.printToFile();
            }
        } catch (Exception exception) {
