...
"""
Code for the math problem presented in this video:
https://www.youtube.com/watch?v=V0V3LMK40iI
"""
...
def redraw(draw_text=False, win=False):
    ...
    pygame.draw.circle(
        window, (255, 255, 255),
        (int(width / 2 + boatx), int(height / 2 + boaty)),
        6, 2
    )
    pygame.draw.circle(
        window, (255, 0, 0),
        (int(width / 2 + radius * math.cos(goblin)), int(height / 2 + radius * math.sin(goblin))),
        6, 0
    )
    ...
    if draw_text:
        ...
        if win:
            ...
            text = font.render("You Were Eaten", True, (255, 0, 0))
    ...
    text = font.render("Goblin Speed: " + str(gspeeds[gspeed_ix]), True, (255, 255, 255))
    textpos = text.get_rect()
...
def detectWin():
    ...
    if boatx * boatx + boaty * boaty > radius * radius:
        ...
        while True:
            ...
            redraw(True, is_win)
            events = [_event.type for _event in pygame.event.get()]
